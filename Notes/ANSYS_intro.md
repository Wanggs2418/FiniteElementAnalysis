...
> [Plastic Equivalent Strain, PEEQ](https://www.jishulink.com/post/178504)
...
- 接触面的大小，相互位置，接触状态等都是随时间不断变化的；
- 接触物体原则上不应互相侵入，法向只存在压力，切向存在摩擦条件，具有强烈的非线性；
...
对于模拟不分离或绑定接触，用户需要设置**接触分离刚度 FKOP** 实常数，会在接触分离时提供一个刚度系数，默认为 1。

- 在接触分离时，产生“回拉”力，为减少分离，需定义一个较大的 FKOP 值；
- 希望接触面分离，但需要在接触面之间建立联系阻止刚体运动，则可以指定较小的 FKOP 值；
...
而 2021R1 版之后则包括对 `csys` 编号的压缩，因此压缩编号时，建议分别指定对应的元素对象，而非直接用 `numcmp,all` 代替，否则会出现意想不到的错误。
...
此处以位移作为数据输出演示，同理应力，应变等也可采用此流程导出。
